...
void HAL_LCD_PortInit(void)
{
    ...
    // LCD_MOSI
    LCD_MOSI_PORT -> SEL0 |=  LCD_MOSI_PIN;//NON IO FUNCTION, SO THESE PINS CAN BE CONTROLLED BY SPI
...
void HAL_LCD_SpiInit(void)
{
    ...
    EUSCI_B0->CTLW0 = EUSCI_B_CTLW0_CKPH |  //PHASE OF 1
                    ...
                    EUSCI_B_CTLW0_MST | //SET AS SPI MASTER. We are master
                    EUSCI_B_CTLW0_MODE_0 | //3 PIN SPI MODE
                    EUSCI_B_CTLW0_SYNC  | // SET AS SYNCRONOUS MODE. SPI is syncronus
                    EUSCI_B_CTLW0_SSEL__SMCLK | //USE SMCLK. clock is the core frew
                    //EUSCI_B_CTLW0_STEM | // UCxSTE digital output, not necessary
                   EUSCI_B_CTLW0_SWRST; //remain eUSCI state machine in reset. keep it in reset
...
void task_lcd_draw_image(void *pvParameters)
{
 ...
 status = xQueueReceive(Queue_LCD, &curLCDimage, portMAX_DELAY);//so we can print the current position while at center

  if(accelDir() != CENTER){ // IF not in center, update the position

        status = xQueueReceive(Queue_LCD, &curLCDimage, portMAX_DELAY);
        x0 = curLCDimage.x - (curLCDimage.image_width_pixels/2);
        x1 = curLCDimage.x + (curLCDimage.image_width_pixels/2);
            if( (curLCDimage.image_width_pixels & 0x01) == 0x00)
                {
                    x1--;
                }

        y0 = curLCDimage.y  - (curLCDimage.image_height_pixels/2);
        y1 = curLCDimage.y  + (curLCDimage.image_height_pixels/2) ;
        if( (curLCDimage.image_height_pixels & 0x01) == 0x00)
            {
                y1--;
            }
        ...
        bytes_per_row = curLCDimage.image_width_pixels / 8;
        if( (curLCDimage.image_width_pixels % 8) != 0)
        {
            bytes_per_row++;
        }

        for (i=0;i< curLCDimage.image_height_pixels ;i++)
        {
            for(j= 0; j < curLCDimage.image_width_pixels; j++)
                {
                if( (j %8) == 0)
                {
                    byte_index = (i*bytes_per_row) + j/8;
                    data = curLCDimage.image[byte_index];
                }
                if ( data & 0x80)
                {
                    HAL_LCD_writeData(curLCDimage.fColor >> 8);
                    HAL_LCD_writeData(curLCDimage.fColor);
                }
                else
                {
                    HAL_LCD_writeData(curLCDimage.bColor >> 8);
                    HAL_LCD_writeData(curLCDimage.bColor);
                }
                data  = data << 1;
                }
        }
